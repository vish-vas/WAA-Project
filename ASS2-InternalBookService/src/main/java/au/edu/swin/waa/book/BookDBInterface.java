...
public class BookDBInterface 
{
	...
	public final String BOOKISBN13_COLUMN = "book_isbn13";
	...
	public String getBookFromDB(String bookIsbn)
	{
		try
		{
			...
			{
				pStmt = conn.prepareStatement("select * from "+BOOKDB_TABLE_NAME+" where "+BOOKISBN13_COLUMN+" = ?");
			}
			else if(bookIsbn.length() == 10)
			{
				pStmt = conn.prepareStatement("select * from "+BOOKDB_TABLE_NAME+" where "+BOOKISBN_COLUMN+" = ?");
			}
			...
			while(rSet.next())
			{
				...
				tempBook+=rSet.getString(BOOKISBN13_COLUMN)+SEPARATER;
